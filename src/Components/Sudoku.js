...
class Sudoku extends React.Component {
    ...
    sudokuSolver(board) {
        const emptySquare = this.getEmptySquare(board);
        if (!emptySquare) {
            return true;
        }
        const [i, j] = emptySquare;
        const candidates = this.getAllowedNumbers(i, j, board);
    ...
    renderSquare(i, j) {
        ...
        return <Square
            className={classname}
            value={this.state.board[i][j]?this.state.board[i][j]:""}
            onClick={() => {
